...
(check `.agent/skills/<resolved-name>/SKILL.md` or `skills/<resolved-name>/SKILL.md`).
...
- If evolving a **specific skill** (e.g., `/aif-evolve plan`) → read only that one:
  `Read: .agent/skills/aif-plan/SKILL.md` (or `skills/aif-plan/SKILL.md` if not installed)
- If evolving **all skills** (`/aif-evolve` or `/aif-evolve all`) → read all:
  `Glob: .agent/skills/*/SKILL.md` (or `Glob: skills/*/SKILL.md` if not installed)
